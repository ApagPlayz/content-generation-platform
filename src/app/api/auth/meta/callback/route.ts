...
export async function GET(req: Request) {
  ...
  if (error) {
    return NextResponse.redirect(`${base}/settings?meta_error=${encodeURIComponent(error)}`)
  }
  if (!code) {
    return NextResponse.redirect(`${base}/settings?meta_error=missing_code`)
  }

  try {
    const { handle } = await exchangeAndStore(code)
    return NextResponse.redirect(`${base}/settings?meta_connected=${encodeURIComponent(handle)}`)
  } catch (e) {
