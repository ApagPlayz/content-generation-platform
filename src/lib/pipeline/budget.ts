...
//
// This is a between-stages stop, not a real-time ceiling: a stage already in
// flight finishes before the next pre-stage check, so final spend can land a
// touch over the cap — never far past it. The guarantee delivered is: once
// cumulative spend reaches the cap, no further stage starts.
//
 ...
 * deliberate budget stop apart from a genuine crash — and skip retrying it.
 */
 ...
 * cap (null/undefined), a non-finite cap (NaN), or a non-positive cap
 * (0/negative) all mean "no cap" and never stop a run: the create-agent form
 * stores null for an empty field, so treating 0 as "block everything" would
 * silently brick every agent. The boundary is inclusive (>=) on purpose — the
 * check runs BEFORE the next paid stage, so a run that has reached its cap must
 * halt before that stage bills past it.
 */
